...
impl<'a> ServerKey<'a> {
    fn parse(server_key_bytes: &'a [u8]) -> Result<Self, crate::Error> {
        if let Ok(public_key) = parse_public_key(server_key_bytes) {
            return Ok(Self::PublicKey(public_key));
        }

        if let Ok(certificate) = Certificate::from_bytes(server_key_bytes) {
            return Ok(Self::Certificate(certificate));
        }

        Ok(Self::Raw(server_key_bytes))
    }

    fn subject_public_key(&self) -> Result<PublicKey, crate::Error> {
        match self {
            Self::PublicKey(public_key) => Ok(public_key.clone()),
            Self::Certificate(certificate) => Ok(certificate.public_key().clone().into()),
            Self::Raw(_) => Err(crate::Error::CouldNotReadKey),
        }
    }

    fn encoded_bytes(&self) -> Result<CryptoVec, crate::Error> {
        match self {
            Self::PublicKey(public_key) => public_key
                .to_bytes()
                .map(Into::into)
            ...
            Self::Certificate(certificate) => certificate
                ...
                .map(Into::into)
                .map_err(crate::Error::from),
            Self::Raw(server_key_bytes) => Ok(server_key_bytes.to_vec().into()),
        }
    }
}
...
impl KexDhDone {
    async fn server_key_check<H: Handler, R: Reader>(
        ...
        let server_key_bytes = map_err!(Bytes::decode(r))?; // server public key or certificate.
        let server_key = ServerKey::parse(&server_key_bytes)?;
        let pubkey = server_key.subject_public_key()?;
        debug!("server_public_Key: {:?}", pubkey);
        if !rekey {
            let check = handler
                .check_server_key_ext(&pubkey, &server_key_bytes)
                .await?;
            if !check {
        ...
        HASH_BUFFER.with(|buffer| {
            ...
            let hash = {
                ...
                let mut server_key_vec = CryptoVec::new();
                map_err!(server_key.encoded_bytes()?.encode(&mut server_key_vec))?;

                let hash =
                    self.kex
                        .compute_exchange_hash(&server_key_vec, &self.exchange, &mut buffer)?;
...
pub trait Handler: Sized + Send {
    ...
    async fn check_server_key_ext(
        ...
        self.check_server_key(server_public_key).await
    }
