...
ensureGuYanElements();
const props = withDefaults(defineProps<{ modelValue: string; placeholder?: string; disabled?: boolean; readonly?: boolean; id?: string; rows?: number; spellcheck?: boolean | 'true' | 'false'; autocorrect?: string; autocapitalize?: string; resize?: 'none' | 'vertical' | 'horizontal' | 'both'; }>(), { placeholder: '', disabled: false, readonly: false, id: '', rows: 3, spellcheck: undefined, autocorrect: undefined, autocapitalize: undefined, resize: 'vertical' });
...
<template><gt-textarea ref="element" :value="props.modelValue" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :id="id" :rows="rows" :spellcheck="spellcheck" :autocorrect="autocorrect" :autocapitalize="autocapitalize" :resize="resize" v-bind="$attrs" @gt-input="emit('update:modelValue', valueFrom($event))" @gt-change="emit('change', valueFrom($event))" @focus="emit('focus', $event as FocusEvent)" @blur="emit('blur', $event as FocusEvent)" @keydown="emit('keydown', $event as KeyboardEvent)" /></template>
