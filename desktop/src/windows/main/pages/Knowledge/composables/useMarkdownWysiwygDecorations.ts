...
class BlockPreviewWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    container.addEventListener('mousedown', (event) => {
      if ((event.target as HTMLElement | null)?.closest('.cm-md-live-block__action, .cm-md-live-callout__type, .cm-md-live-table__cell')) return;
      event.preventDefault();
      event.stopPropagation();
    });
    container.addEventListener('click', (event) => {
      ...
      clearSelectedLivePreviewBlocks(view.dom);
      container.classList.add(selectedBlockClass);
    });
    ...
    if (this.token.kind === 'table') {
      createTablePreview(container, view, this.token);
      observeBlockSize(container, view);
      requestAnimationFrame(() => view.requestMeasure());
      return container;
    }
...
function clearSelectedLivePreviewBlocks(root: ParentNode) {
  root.querySelectorAll<HTMLElement>(`.${selectedBlockClass}`).forEach((block) => {
    block.classList.remove(selectedBlockClass);
  });
}
...
function createCodeFenceRenderedPreview(container: HTMLElement, token: MarkdownLivePreviewBlockToken) {
  ...
  const header = createCodeFenceFenceRow(preview.openingLineNumber, preview.openingFence);
  header.classList.add('cm-md-live-code-render-preview__header');

  const body = document.createElement('div');
  body.className = 'cm-md-live-code-render-preview__body';
  preview.lines.forEach((line, lineNumber) => {
    body.append(createCodeFencePreviewLine(line, preview.bodyStartLineNumber + lineNumber, preview.language));
  });

  const footer = createCodeFenceFenceRow(preview.closingLineNumber, preview.closingFence);
  footer.classList.add('cm-md-live-code-render-preview__footer');
...
function extractCodeFencePreview(token: MarkdownLivePreviewBlockToken) {
  ...
  return {
    ...
    openingLineNumber: 1,
    openingFence: openingLine || `\`\`\`${token.language || parsedLanguage}`,
    bodyStartLineNumber: 2,
    closingLineNumber: closingLineIndex + 1,
    closingFence: closingLine,
    lines: lines.length > 0 ? lines : [''],
  };
}

function createCodeFenceFenceRow(lineNumber: number, source: string) {
  const row = document.createElement('div');
  row.className = 'cm-md-live-code-render-preview__fence-row';
  ...
  code.className = 'cm-md-live-code-render-preview__code cm-md-live-code-render-preview__fence-source';
...
function createTablePreview(container: HTMLElement, view: EditorView, token: MarkdownLivePreviewBlockToken) {
  const parsed = parseMarkdownTable(token.source);
  const width = Math.max(...parsed.rows.map((row) => row.length), parsed.alignments.length, 1);
  const rows = parsed.rows.map((row) => Array.from({ length: width }, (_item, index) => row[index] ?? ''));

  const header = document.createElement('div');
  header.className = 'cm-md-live-block__header cm-md-live-table__header';
  const title = document.createElement('span');
  title.className = 'cm-md-live-block__title';
  title.textContent = blockTitle(token);
  header.append(title);
  header.append(createBlockButton('源码', 'edit', view, token));
  header.append(createBlockButton('行+', 'table-row', view, token));
  header.append(createBlockButton('列+', 'table-column', view, token));
  header.append(createBlockButton('居中', 'table-align-center', view, token));

  const shell = document.createElement('div');
  shell.className = 'cm-md-live-table-shell';

  const table = document.createElement('table');
  table.className = 'cm-md-live-table';

  rows.forEach((row, rowIndex) => {
    const tr = document.createElement('tr');
    row.forEach((value, columnIndex) => {
      const cell = document.createElement(rowIndex === 0 ? 'th' : 'td');
      cell.className = 'cm-md-live-table__cell';
      cell.contentEditable = 'plaintext-only';
      cell.spellcheck = false;
      cell.textContent = value;
      cell.dataset.tableRow = String(rowIndex);
      cell.dataset.tableColumn = String(columnIndex);
      cell.setAttribute('aria-label', `表格单元格 ${rowIndex + 1}, ${columnIndex + 1}`);
      cell.addEventListener('blur', () => {
        const nextValue = cell.textContent ?? '';
        if (nextValue === value) return;
        const nextSource = updateMarkdownTableCell(token.source, rowIndex, columnIndex, nextValue);
        view.dispatch({
          changes: { from: token.from, to: token.to, insert: nextSource },
          scrollIntoView: true,
        });
  ...
  resizeHandle.className = 'cm-md-live-table__resize-handle';
  resizeHandle.setAttribute('aria-hidden', 'true');

  shell.append(table, resizeHandle);
    ...
    provide: (field) => [
      ...
      EditorView.domEventHandlers({
        mousedown(event, view) {
          if ((event.target as HTMLElement | null)?.closest('.cm-md-live-block')) return;
          clearSelectedLivePreviewBlocks(view.dom);
        },
        blur(event, view) {
          const relatedTarget = (event as FocusEvent).relatedTarget;
          if (relatedTarget instanceof Element && relatedTarget.closest('.cm-md-live-block')) return;
          clearSelectedLivePreviewBlocks(view.dom);
        },
      }),
