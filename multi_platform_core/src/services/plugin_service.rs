...
use crate::models::{CreatePluginFileGrantInput, CreatePluginJobInput, PluginFileGrant, PluginInstallation, PluginJob, PluginMarketplaceCache, UpdatePluginJobInput, UpsertPluginInstallationInput, UpsertPluginMarketplaceInput};
use rusqlite::{params, Connection, OptionalExtension};

pub struct PluginService;

impl PluginService {
    pub fn upsert_installation(db: &Database, input: UpsertPluginInstallationInput) -> DbResult<PluginInstallation> {
        if input.plugin_id.trim().is_empty() || input.current_path.trim().is_empty() {
            return Err(DbError::InvalidParameter("plugin installation id and current path are required".to_string()));
        }
        db.transaction(|conn| {
            conn.execute(
    ...
    pub fn get_installation(db: &Database, plugin_id: &str) -> DbResult<Option<PluginInstallation>> {
    ...
    pub fn delete_installation(db: &Database, plugin_id: &str) -> DbResult<()> {
        db.transaction(|conn| {
            conn.execute("DELETE FROM plugin_installations WHERE plugin_id = ?1", params![plugin_id])?;
            Ok(())
        })
    }

    pub fn create_file_grant(db: &Database, input: CreatePluginFileGrantInput) -> DbResult<PluginFileGrant> {
        if input.id.trim().is_empty() || input.plugin_id.trim().is_empty() || input.root_path.trim().is_empty() {
            return Err(DbError::InvalidParameter("plugin file grant fields are required".to_string()));
        }
        db.transaction(|conn| {
            conn.execute(
    ...
    pub fn revoke_file_grant(db: &Database, plugin_id: &str, id: &str) -> DbResult<()> {
        db.transaction(|conn| {
            let changed = conn.execute("UPDATE plugin_file_grants SET revoked = 1 WHERE id = ?1 AND plugin_id = ?2", params![id, plugin_id])?;
            if changed == 0 { return Err(DbError::NotFound(id.to_string())); }
            Ok(())
        })
    }
    ...
    pub fn upsert_marketplace(db: &Database, input: UpsertPluginMarketplaceInput) -> DbResult<PluginMarketplaceCache> {
        db.transaction(|conn| {
            conn.execute(
    ...
    pub fn create_job(db: &Database, input: CreatePluginJobInput) -> DbResult<PluginJob> {
        if input.id.trim().is_empty() || input.plugin_id.trim().is_empty() || input.kind.trim().is_empty() {
            return Err(DbError::InvalidParameter("plugin job id, plugin id, and kind are required".to_string()));
        }
        ...
        db.transaction(|conn| {
            conn.execute(
                ...
                params![input.id, input.plugin_id, input.parent_job_id, input.kind, input.input_json],
            )?;
    ...
    pub fn update_job(db: &Database, id: &str, input: UpdatePluginJobInput) -> DbResult<PluginJob> {
        db.transaction(|conn| {
            ...
            Self::get_job_conn(conn, id)?.ok_or_else(|| DbError::NotFound(format!("plugin job {}", id)))
        })
    }
    ...
    pub fn retry_job(db: &Database, source_id: &str, new_id: &str) -> DbResult<PluginJob> {
        let source = Self::get_job(db, source_id)?.ok_or_else(|| DbError::NotFound(source_id.to_string()))?;
        Self::create_job(db, CreatePluginJobInput {
            id: new_id.to_string(), plugin_id: source.plugin_id, parent_job_id: Some(source.id), kind: source.kind, input_json: source.input_json,
        })
    }
    ...
    fn get_installation_conn(conn: &Connection, plugin_id: &str) -> DbResult<Option<PluginInstallation>> {
...
mod tests {
    ...
    fn creates_and_lists_plugin_jobs_by_owner() {
        ...
        assert_eq!(PluginService::list_jobs(&db, "guyantools.example").unwrap().len(), 1);
        assert!(PluginService::list_jobs(&db, "other.plugin").unwrap().is_empty());
    }
    ...
    fn owns_file_grants_and_marketplace_cache() {
        let db = Database::new_in_memory().unwrap();
        let grant = PluginService::create_file_grant(&db, CreatePluginFileGrantInput {
            id: "grant-1".into(), plugin_id: "plugin.one".into(), purpose: "data".into(),
            root_path: "C:/data".into(), access_mode: "read-write".into(), expires_at: "2099-01-01T00:00:00Z".into(), max_bytes: 1024,
        }).unwrap();
        ...
        let cache = PluginService::upsert_marketplace(&db, UpsertPluginMarketplaceInput {
            id: "official".into(), url: "https://example.com/catalog.json".into(), ref_name: "main".into(),
            catalog_json: "{}".into(), catalog_sha256: "abc".into(), enabled: true,
        }).unwrap();
    ...
    fn upserts_installations_and_retries_with_a_child_job() {
        let db = Database::new_in_memory().unwrap();
        let installation = PluginService::upsert_installation(&db, UpsertPluginInstallationInput {
            plugin_id: "plugin.one".into(), manifest_json: "{}".into(), source_json: "{}".into(),
            resolved_commit: Some("abc1234".into()), package_sha256: None, approved_permissions_json: "[]".into(), capabilities_json: "[]".into(), current_path: "C:/current".into(), previous_path: None, status: "resolved".into(),
        }).unwrap();
        assert_eq!(installation.resolved_commit.as_deref(), Some("abc1234"));
        assert!(PluginService::get_installation(&db, "plugin.one").unwrap().is_some());

        let job = PluginService::create_job(&db, CreatePluginJobInput {
            id: "job-parent".into(), plugin_id: "plugin.one".into(), parent_job_id: None, kind: "download".into(), input_json: "{}".into(),
        }).unwrap();
