...
- Stencil 迁移组件的样式边界：Vue 包装组件传递到 `gt-*` host 的是业务 class（例如 `.home-profile-switcher__trigger`），不要假设旧 `.ui-button`/`.ui-icon-button` 类会存在；Shadow DOM 内部元素必须通过组件公开的 `::part(...)` 选择器覆盖，组件提供的 `prefix`/默认插槽/`suffix` 等 named slot 应按语义分别放置内容，不要把多个布局元素全部塞入单一默认 slot。
