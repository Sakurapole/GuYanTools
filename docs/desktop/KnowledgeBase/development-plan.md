...
知识库内置一个轻量问答入口，满足“边看资料边提问”的基本场景。它复用现有 AI Chat 的助手角色、Provider、模型、流式输出、引用和停止生成能力，不在知识库内重复实现 Provider 管理或独立聊天协议。复杂交互继续进入 AI 页面。
...
- 复用 AI Chat 助手角色：
  - 可选择 AI 页面已配置的助手角色。
  - 助手绑定 Provider / 模型时优先使用助手配置。
  - 助手绑定知识库范围时优先使用助手的 library / space。
  - 未绑定时使用 AI Chat 默认 Provider / 模型和当前知识库范围。
- 选区动作：
  - 总结。
  - 改写。
  - 翻译。
...
- 空间动作：
  - 当前空间问答。
- 知识库内部问答：
  - 在右侧 Inspector AI tab 内直接输入问题。
  - 支持当前页面、当前空间、当前库、选区四种发送范围。
  - 默认使用 `knowledgeSearchMode = force` 检索知识库上下文。
  - 回答必须展示页面、块、附件或文档来源引用。
  - 支持停止生成、重新生成、复制回答。
  - 支持“在 AI 页面继续”，携带问题、回答、引用、助手、Provider、模型和知识库范围跳转。
- AI 输出预览和手动确认。
- AI 操作日志。
- embedding 管理：
  - 使用现有 `knowledge_ai_chunks` 和 `knowledge_embeddings`。
  - 使用现有 AI embedding 服务生成向量。
  - 用户显式触发当前库或当前空间 embedding 重建。
  - 可查看 chunk / embedding 统计。
  - 可删除并重建指定 Provider / 模型的 embedding。

#### 交互边界

- 基础问答、页面摘要、标签建议和选区处理留在知识库内完成。
- 深度研究、多轮长上下文整理、Canvas 工作区、Agent 工具调用、跨页面批量修改进入 AI 页面。
- 知识库内 AI 不自动写入正文；写入页面、Todo 或标签前必须展示预览和确认。
- 知识库回答没有引用时只能作为普通模型回答展示，不标记为可信知识库回答。

#### 验收标准

- 未配置 AI 时功能不报错。
- 已配置 AI 时，可在知识库 Inspector 内选择助手角色、Provider 和模型并完成基础问答。
- 基础问答不需要离开知识库页面。
- “在 AI 页面继续”能保留当前上下文和引用。
- AI 结果不会自动覆盖用户内容。
- 所有问答结果展示来源引用。
- embedding 重建必须由用户显式触发，且可删除、可重建。
- 用户可关闭 AI 入口。
