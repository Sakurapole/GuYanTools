...
**Architecture:** `@guyantools/ui-core` owns Stencil component source, `--gt-*` tokens, loader, generated element types and Vue proxies. `@guyantools/plugin-ui` receives the Stencil-generated React proxies and publishes them from its stable `./react` entrypoint. `@guyantools/ui-vue` consumes the generated Vue proxies and supplies only the existing Vue-specific `v-model`, slot, expose-method and overlay compatibility wrappers.
...
- Modify `packages/ui-core/package.json`, `tsconfig.json`, and replace `vite.config.ts` with `stencil.config.ts`: Stencil compiler, `dist/custom-elements`, loader, Vue proxy and React proxy output targets. Use `@stencil/vitest`, not the deprecated integrated Stencil Jest runner.
...
- [x] **Step 1: Write the failing Stencil build contract.**
...
- [x] **Step 2: Verify the test fails before Stencil is configured.**

Run: `pnpm --dir packages/ui-core exec stencil-test --project spec src/components/core-build.spec.tsx`

Expected: FAIL because `@stencil/core` and `stencil.config.ts` do not exist.

- [x] **Step 3: Configure the package and output targets.**
...
export const config: Config = {
  ...
  srcDir: 'src',
  outputTargets: [
    { type: 'dist-custom-elements', dir: 'dist/custom-elements', externalRuntime: false, generateTypeDeclarations: true },
    vueOutputTarget({
...
- [x] **Step 4: Verify Stencil build and generated Vue proxies.**
...
Test-Path packages/plugin-ui/src/generated/react
```

Expected: all specs PASS and all generated output paths exist.

- [x] **Step 5: Commit the Stencil compiler boundary.**
