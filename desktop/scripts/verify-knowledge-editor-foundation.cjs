...
function testKnowledgeMarkdownEditorUsesCodeMirrorImplementation() {
  ...
  assert.match(
    wysiwygDecorationsSource,
    /function createCodeFenceRenderedPreview[\s\S]*?cm-md-live-code-render-preview__fence-row[\s\S]*?cm-md-live-code-render-preview__footer/,
  ...
  assert.match(
    wysiwygDecorationsSource,
    /function createCodeFencePreviewLine[\s\S]*?cm-md-live-code-render-preview__line-number[\s\S]*?String\(lineNumber \+ 1\)[\s\S]*?cm-md-live-code-render-preview__code/,
    'expected code fence preview rows to include stable one-based line numbers',
  );
  assert.match(
    wysiwygDecorationsSource,
  ...
  assert.match(
    editorSource,
    /\.cm-md-live-code-render-preview__fence-row[\s\S]*?font-family:\s*inherit;[\s\S]*?font-size:\s*inherit;[\s\S]*?line-height:\s*inherit;[\s\S]*?\.cm-md-live-code-render-preview__footer/,
  ...
  assert.match(
    editorSource,
    /\.cm-md-live-source-block--code_fence[\s\S]*?background:\s*color-mix\(in srgb, var\(--ui-surface-panel-muted\)[\s\S]*?box-shadow:\s*none;/,
    'expected active code fence source blocks to use the same subtle background as previews',
  );
  assert.match(
    editorSource,
