...
const status = ref<AndroidInputStatus>({ state: 'windows', running: false, deviceSerial: '', virtualCursor: { x: 0, y: 0 } });
const error = ref('');
...
const errorLabels: Record<string, string> = { ANDROID_DEVICE_NOT_FOUND: '当前设备不可用，请在设备信息中选择一台已授权设备。', ANDROID_INPUT_BRIDGE_UNAVAILABLE: '系统输入桥接不可用，请确认当前运行环境为 Windows x64。', ANDROID_UHID_START_FAILED: 'Android 输入服务启动失败，请检查 USB 调试和设备授权，并确认工具链包含 UHID 服务。', ANDROID_UHID_DISCONNECTED: 'Android 输入服务已断开，Windows 键鼠已恢复。', ANDROID_TOOL_UNAVAILABLE: 'Android 工具链缺少 UHID 服务文件，请重新安装或更新工具链。' };
