...
这会在 `multi_platform_core/` 目录生成当前平台对应的 `.node` 文件，并自动带上 `--features napi`。
...
```json
{
  ...
  "dependencies": {
    "@guyantools/core": "workspace:*"
  }
}
```

Electron Forge 打包时应启用 `@electron-forge/plugin-auto-unpack-natives`，不要再用 `extraResource` 从源码目录复制 `.node`。
...
**问题**：找不到 `.node` 模块
- 确保已运行 `pnpm --dir multi_platform_core build`
- 确保依赖使用 `@guyantools/core: workspace:*`
- 确保 Forge 启用了 `@electron-forge/plugin-auto-unpack-natives`
