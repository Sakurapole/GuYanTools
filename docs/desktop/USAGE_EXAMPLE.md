...
`@guyantools/core` 通过 `pnpm workspace` 自动链接，不再依赖手工复制 `.node` 文件。
...
1. **构建命令**：必须通过 `napi build --features napi` 生成 `.node`，仓库已封装为 `pnpm run native:build` 和 `pnpm run native:build:debug`
